...
def home():
    return jsonify({
        "status": "online",
        "message": "Meanny AI Video Voice Dubber Backend is running successfully!"
    }), 200
...
def generate_dub():
    ...
    if video_file:
        ...
        resolved_video = resolve_existing_path(data['video_path'])
    ...
    if not srt_path.exists():
        return jsonify({"status": "error", "message": "រកមិនឃើញឯកសារ SRT ទេ។"}), 400

    output_filename = f"dubbed_output_{int(time.time())}.mp4"
    ...
    try:
        ...
        update_dubbing_status(status='error', progress=0, message=str(e))
...
def translate_srt():
    srt_file = request.files.get('srt')
    srt_content = None if srt_file else request.form.get('srt_content')
    if not srt_file and not srt_content:
        return jsonify({"status": "error", "message": "Missing SRT file or content."}), 400

    data = request.json or request.form or {}
    voice_option = normalize_voice_option(data.get('voice_option', data.get('selected_voice', 'auto')))
