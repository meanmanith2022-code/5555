...
class _WatermarkEditorScreenState extends State<WatermarkEditorScreen> {
  ...
  final TextEditingController _textController =
      TextEditingController(text: 'Subtitles / Text');
  ...
  // ✅ កែប្រែវិធីហៅប្រើ FilePicker ឱ្យត្រឹមត្រូវ ១០០%
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final String newVideoUrl =
            data['video_url'] as String? ?? widget.videoPath ?? '';

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Successfully sent data to Python Backend!'),
          ),
        );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Mock Data Saved: Server not connected (${e.toString()})'),
        ),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor:
          isDarkMode ? const Color(0xFF1E1B18) : const Color(0xFFF5F5F5),
      appBar: AppBar(
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    ...
                    onPressed: _isProcessing
                        ? null
                        : () => _sendToPythonBackend(applyWatermark: false),
                    child: const Text(
                      'No Watermark',
                      style: TextStyle(
                        color: Color(0xFF3B99F5),
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: _isProcessing
                        ? null
                        : () => _sendToPythonBackend(applyWatermark: true),
                    child: _isProcessing
                        ? const SizedBox(
                            width: 20,
                            height: 20,
              ...
              children: [
                ...
                CircleAvatar(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () => setState(() =>
                        logoScale = (logoScale > 0.5) ? logoScale - 0.1 : 0.5),
                  ),
                ),
                const SizedBox(width: 8),
                CircleAvatar(
                  ...
                  child: IconButton(
                    icon: const Icon(Icons.add,
                        color: Colors.black, size: 16),
                    onPressed: () => setState(() =>
                        logoScale = (logoScale < 2.5) ? logoScale + 0.1 : 2.5),
                  ),
                ),
              ],
            ),
              ...
              children: [
                ...
                IconButton(
                  icon: Icon(
                    ...
                    color: const Color(0xFF3B99F5),
                  ),
                  onPressed: () {
                    setState(() {
                      isMuted = !isMuted;
                      _videoController?.setVolume(
                        isMuted ? 0.0 : videoVolume,
                      );
                    });
                  },
                ),
  ...
  Widget _buildDraggableElement({
    ...
    final maxY =
        (containerHeight - elementHeight).clamp(0.0, containerHeight);
  ...
  Widget _buildMenuButton(
      int index, IconData icon, String label, bool isDarkMode) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? const Color(0xFF3B99F5)
              : (isDarkMode ? const Color(0xFF262320) : Colors.white),
          borderRadius: BorderRadius.circular(12),
        ),
